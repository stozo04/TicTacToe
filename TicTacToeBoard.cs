...
namespace TicTacToe
{
    ...
    /// </summary>
    public class TicTacToeBoard : GridBasedBoard
    {
        /// <summary>
        ...
        public TicTacToeBoard()
        {
            ...
            for (int y = 0; y < Height; y++)
            {
                ...
                {
                    possibleMoves.Add(new TicTacToeMove(x, y));
                }
            }
        }
        ...
        public override void Draw()    {
            int rows = boardContents.GetLength(0);
            int columns = boardContents.GetLength(1);
            // Draw the board
            for (int row = 0; row < rows; row++)
            {
                for (int column = 0; column < columns; column++)
                {
                    Console.Write(boardContents[row, column]);
                    if (column < columns - 1)
                    {
                        Console.Write(" | ");
                    }
